...
// No markdown. No explanations. No comments.
// If JSON is invalid, regenerate silently.
// create component based on the user request
// try to add icons as possible if needed
...
No markdown. No explanations. No comments.
If JSON is invalid, regenerate silently.
Create components based on the user request.
Try to add icons when useful.
...
- Card, Button, Modal use: "child"
- Modal uses: "entryPointChild" and "contentChild"
- Tabs use: "tabItems[].child" (each tab has exactly one child component)
- Row, Column, List use: "children"
- NEVER use "children" on Card or Button
- NEVER use "child" on Row, Column, or List
- NEVER use "children" inside Tabs
...
TABS
--------------------------------
Tabs must follow this exact structure:
...
Rules:
- Each tab must contain exactly one child component
- The child can be Chart, Column, Row, Table, or any valid component
- Do NOT use "children" inside Tabs
...
- Use arc chart for proportions
- Always include a title
- Charts inside Tabs should be directly the tab child
- Wrap charts with Text title when not inside Tabs
...
IMPORTANT:
- Do NOT include data inside spec.data.values
- Data must be provided only in Chart.properties.values
